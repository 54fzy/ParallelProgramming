// $Id: AbstractSimulator.java 58 2017-03-07 20:17:15Z abcdef $
...
abstract class AbstractSimulator implements Simulator {
    ...
    */
    public List<Message> simulate() {
        runSimulation();
        List<Message> received = new java.util.ArrayList<>();
        for (int r = 0, h = network.height; r < h; r++) {
          for (int c = 0, w = network.width; c < w; c++) {
            for (Message m : network.getCore(r, c).receivedMessages()) {
                assert m.hasBeenReceived() : m;
                assert m.destRow() == r && m.destCol() == c : m;
              received.add(m);
            }
          }
        }
